...
def p_write_error(p):
   ...
   '''
    lista_errores.append(
        "ERROR: Error de sintaxis en la función {0} en la línea {1}, falta un ';'".format(p[1], p.lineno(1)))
...
def p_funciones(p):
   ...
   '''
    ...
    if p[2] in palabras_reservadas.values():
        lista_errores.append(
            "ERROR: La función {0} no se pudo definir en la línea {1} ya que no puede tener el mismo nombre que una palabra reservada".format(
                p[2], p.lineno(2)))
    ...
    if p[2] not in funciones:
        ...
        for i in funciones[p[2]]:
            ...
            if len(lista_funciones[i][0]) == len(p[4]):
                lista_errores.append(
                    "ERROR: La función {0} no se pudo definir en la línea {1} ya que está duplicada".format(p[2],
                                                                                                            p.lineno(
                                                                                                                2)))
...
def p_funciones_error(p):
   ...
   '''
    lista_errores.append(
        "ERROR: Error de sintaxis en la definición de procedimiento en la línea {0}".format(p.lineno(2)))
